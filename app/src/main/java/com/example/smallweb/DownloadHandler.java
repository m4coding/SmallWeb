...
public class DownloadHandler {
	...
	public static void onDownloadStart(Activity activity, String url,
		...
		// 当contentDisposition为null或者前面10字符不为attachment时
	 ...
	 */
	/* package */
	static void onDownloadStartNoStream(Activity activity, String url,
			String userAgent, String contentDisposition, String mimetype,
			boolean privateBrowsing) {
		...
		try {
			...
			// exception here
			Log.e(LOGTAG, "Exception while trying to parse url '" + url + '\'',
					e);
			return;
		}
		...
		// 这里是设置将文件下载到哪个目录中，先从settings.xml文件中查找字段为download的字符串，若没有则设置默认的为Download
